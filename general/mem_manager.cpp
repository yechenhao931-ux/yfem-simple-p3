...
/// The managed host memory space in the serial build.
class ManagedHostMemorySpace : public HostMemorySpace
{
public:
   ManagedHostMemorySpace(): HostMemorySpace() { }

   void Alloc(void **ptr, size_t bytes) override
   { *ptr = std::malloc(bytes == 0 ? 8 : bytes); }
...
/// The page-aligned host memory space in the serial build.
...
public:
   ...
   void Configure()
   {
      ...
      host[static_cast<int>(MT::MANAGED)] = new ManagedHostMemorySpace();

      // Filling the device memory backends, shifting with the device size
      constexpr int shift = DeviceMemoryType;
      device[static_cast<int>(MT::MANAGED)-shift] = new StdDeviceMemorySpace();
