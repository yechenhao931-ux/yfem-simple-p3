...
// Implementation of MFEM's lightweight host memory manager. The device-shaped
// APIs are retained as host-backed compatibility shims for serial code.
...
enum class MemoryType
{
   ...
   HOST_UMPIRE,    ///< Compatibility memory type; unavailable in this build
   HOST_PINNED,    ///< Host memory: pinned (page-locked)
   MANAGED,        ///< Host-backed compatibility memory type
   DEVICE,         ///< Host-backed compatibility memory type
   DEVICE_DEBUG,   /**< Pseudo-device memory; allocated on host from a
                        "device-debug" pool */
   DEVICE_UMPIRE,  ///< Compatibility memory type; unavailable in this build
   DEVICE_UMPIRE_2, ///< Compatibility memory type; unavailable in this build
...
private: // Static methods used by the Memory<T> class
   ...
   /// h_tmp must be already allocated using new T[] when mt is not HOST.
